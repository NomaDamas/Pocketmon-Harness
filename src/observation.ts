...
import { detectPokemonPhase } from "./phase-detector";
import {
...
function formatState(observation: MgbaObservation): string {
  if (!observation.state) {
    return "";
  }

  const harnessPhase = detectPokemonPhase({ observation }).phase;
  return `\n\nCurrent compact Pokémon state:\nharnessPhase: ${harnessPhase}\n${formatPokemonStateObservation(observation.state)}`;
}
